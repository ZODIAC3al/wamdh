...
export default function InstructorProfileScreen() {
  ...
  const { colors, isDark: dark, t, isRtl, user, refreshUser, logout } = useWamdh();
    ...
    <>
      <ScrollView style={{ flex: 1, backgroundColor: bg }} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={styles.bannerContainer}>
          <Pressable onPress={() => pickAndUploadImage("banner")} style={StyleSheet.absoluteFill}>
            ...
            <View style={[styles.cameraOverlay, { backgroundColor: "rgba(0,0,0,0.3)" }]}>
        ...
        <View style={{ paddingHorizontal: 20, marginTop: -40 }}>
          <View style={[styles.profileCard, { backgroundColor: cardBg, borderColor: cardBorder }]}>
            <Pressable onPress={() => pickAndUploadImage("profile")} style={[styles.avatarCircle, { backgroundColor: cardBg, borderColor: cardBg }]}>
              {user?.profile_photo_url ? (
                <Image source={{ uri: user.profile_photo_url }} style={styles.avatarImage} />
              ) : (
                <View style={[StyleSheet.absoluteFill, { backgroundColor: PRIMARY, alignItems: "center", justifyContent: "center" }]}>
                  <Text style={{ color: "#FFFFFF", fontFamily: "Sora_700Bold", fontSize: 28 }}>
                    {user?.username?.charAt(0).toUpperCase() || "I"}
                  </Text>
                </View>
              )}
              <View style={[styles.avatarCameraBtn, { backgroundColor: PRIMARY, right: isRtl ? undefined : 0, left: isRtl ? 0 : undefined }]}>
                <Ionicons name="camera" size={12} color="#FFFFFF" />
              </View>
            </Pressable>

            <Text style={[styles.userName, { color: textPrimary }]}>{user?.username || "Instructor"}</Text>
            <Text style={{ color: textSecondary, fontFamily: "Inter_400Regular", fontSize: 12, marginTop: 2 }}>
              {user?.email || "instructor@wamdh.edu"}
            </Text>
            
            <View style={[styles.badge, { backgroundColor: PRIMARY + "15", flexDirection: isRtl ? "row-reverse" : "row" }]}>
              <Ionicons name="school-outline" size={13} color={PRIMARY} style={{ marginRight: isRtl ? 0 : 4, marginLeft: isRtl ? 4 : 0 }} />
              <Text style={[styles.badgeText, { color: PRIMARY }]}>
                {isRtl ? "معلم معتمد" : "Certified Instructor"}
              </Text>
            </View>

            <Text style={[styles.bioText, { color: textSecondary }]}>
              {user?.bio || (isRtl ? "لم يتم كتابة سيرة ذاتية بعد. اضغط على تعديل لتخصيصها." : "No biography provided. Tap Edit Profile to customize.")}
            </Text>
          </View>
        </View>
        ...
        <View style={{ padding: 20, gap: 12 }}>
          ...
          >
            ...
            <View style={{ flex: 1, marginLeft: isRtl ? 0 : 12, marginRight: isRtl ? 12 : 0, alignItems: isRtl ? "flex-end" : "flex-start" }}>
              ...
              <Text style={{ color: textSecondary, fontFamily: "Inter_400Regular", fontSize: 11, marginTop: 1, textAlign: isRtl ? "right" : "left" }}>
                {isRtl ? "تغيير الصورة الشخصية، الغلاف والسيرة الذاتية" : "Change photo, banner, and bio"}
              </Text>
            </View>
          ...
          >
            ...
            <View style={{ flex: 1, marginLeft: isRtl ? 0 : 12, marginRight: isRtl ? 12 : 0, alignItems: isRtl ? "flex-end" : "flex-start" }}>
              ...
              <Text style={{ color: textSecondary, fontFamily: "Inter_400Regular", fontSize: 11, marginTop: 1, textAlign: isRtl ? "right" : "left" }}>
                {isRtl ? "امسح الرمز للتواصل مع المعلم" : "Show QR code for others to scan"}
              </Text>
            </View>
            <Ionicons name={isRtl ? "chevron-back" : "chevron-forward"} size={16} color={textSecondary} />
          </Pressable>
          ...
          <Pressable
            onPress={() => router.push("/(instructor)/courses" as any)}
            style={[styles.menuItem, { backgroundColor: cardBg, borderColor: cardBorder, flexDirection: isRtl ? "row-reverse" : "row" }]}
          >
            <View style={[styles.menuIcon, { backgroundColor: PRIMARY + "15" }]}>
            ...
            <View style={{ flex: 1, marginLeft: isRtl ? 0 : 12, marginRight: isRtl ? 12 : 0, alignItems: isRtl ? "flex-end" : "flex-start" }}>
              ...
              <Text style={{ color: textSecondary, fontFamily: "Inter_400Regular", fontSize: 11, marginTop: 1, textAlign: isRtl ? "right" : "left" }}>
                {isRtl ? "مراجعة المنهج الدراسي، الملفات والطلاب" : "Review syllabus, files, and enrollments"}
              </Text>
            </View>
            <Ionicons name={isRtl ? "chevron-back" : "chevron-forward"} size={16} color={textSecondary} />
          </Pressable>
          ...
          >
            ...
            <View style={{ flex: 1, marginLeft: isRtl ? 0 : 12, marginRight: isRtl ? 12 : 0, alignItems: isRtl ? "flex-end" : "flex-start" }}>
              ...
              <Text style={{ color: textSecondary, fontFamily: "Inter_400Regular", fontSize: 11, marginTop: 1, textAlign: isRtl ? "right" : "left" }}>
                {isRtl ? "قطع الاتصال بحسابك بشكل آمن" : "Securely disconnect from your account"}
              </Text>
            </View>
            <Ionicons name={isRtl ? "chevron-back" : "chevron-forward"} size={16} color={textSecondary} />
          </Pressable>
        ...
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.6)", justifyContent: "center", alignItems: "center", padding: 24 }}>
          <View style={{ backgroundColor: cardBg, borderRadius: 24, padding: 24, width: "100%", maxWidth: 340, alignItems: "center", borderWidth: 1, borderColor: cardBorder }}>
            ...
            <Text style={{ color: textSecondary, fontFamily: "Inter_400Regular", fontSize: 12, textAlign: "center", marginBottom: 24, paddingHorizontal: 12 }}>
              {isRtl ? "دع الآخرين يمسحون الرمز للتواصل مع حساب المعلم الخاص بك في ومضة" : "Let others scan this code to connect with your Wamdh instructor account"}
            </Text>
...
const styles = StyleSheet.create({
  ...
  cameraOverlay: { position: "absolute", bottom: 10, right: 10, width: 32, height: 32, borderRadius: 16, alignItems: "center", justifyContent: "center" },
  ...
  avatarCircle: { width: 80, height: 80, borderRadius: 40, borderWidth: 4, alignItems: "center", justifyContent: "center", marginTop: -60, overflow: "visible" },
  avatarImage: { width: "100%", height: "100%", borderRadius: 40 },
  avatarCameraBtn: { position: "absolute", bottom: 0, right: 0, width: 22, height: 22, borderRadius: 11, alignItems: "center", justifyContent: "center", borderWidth: 2, borderColor: "#FFFFFF" },
  userName: { fontFamily: "Sora_700Bold", fontSize: 18, marginTop: 10 },
  badge: { flexDirection: "row", alignItems: "center", paddingHorizontal: 10, paddingVertical: 4, borderRadius: 10, marginTop: 8 },
